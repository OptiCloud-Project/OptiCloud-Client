...
export default function StorageCostDetailsModal({ open, onClose, files, totalCost, pricing }) {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle>
        <Typography variant="h6" component="div" sx={{ fontWeight: 600, color: '#111718' }}>
          Storage Cost Details
        </Typography>
      </DialogTitle>
      <DialogContent>
        <Box sx={{ mb: 3 }}>
          <Typography variant="h6" sx={{ mb: 2, color: '#111718', fontWeight: 600 }}>
            Total Cost: ${totalCost.toFixed(2)}
          </Typography>
          
          {/* Tier Summary */}
          <Paper sx={{ p: 2, mb: 3, backgroundColor: '#f5f5f5' }}>
            <Typography variant="subtitle1" sx={{ mb: 1, fontWeight: 600, color: '#111718' }}>
              Summary by Tier:
            </Typography>
            <Box sx={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
              {Object.entries(tierSummary).map(([tier, summary]) => (
                <Box key={tier} sx={{ minWidth: 150 }}>
                  <Typography variant="body2" sx={{ fontWeight: 600, color: '#66898F' }}>
                    {tier} Tier:
                  </Typography>
          ...
          <Box sx={{ mb: 2, p: 1.5, backgroundColor: '#e3f2fd', borderRadius: 1 }}>
            ...
            <Typography variant="body2" sx={{ color: '#666' }}>
              HOT: ${pricing.HOT}/MB | WARM: ${pricing.WARM}/MB | COLD: ${pricing.COLD}/MB
            </Typography>
          </Box>
        </Box>
        ...
        <Table size="small">
          <TableHead>
            <TableRow>
              ...
              <TableCell align="right" sx={{ fontWeight: 600 }}>Tier</TableCell>
              <TableCell align="right" sx={{ fontWeight: 600 }}>Size (MB)</TableCell>
            ...
            {filesWithCost.length === 0 ? (
              ...
              filesWithCost.map((file) => (
                <TableRow key={file.id}>
                  ...
                  <TableCell align="right">{file.sizeInMB.toFixed(2)}</TableCell>
